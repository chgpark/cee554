...
Range-only(RO) SLAM is a method for localizing a mobile robot and beacons by mainly utilizing distance measurements. Because range-only measurements have only magnitude so it has rank-deficiency. And distance is only measured by time of flight(TOA), data is noisy itselves.
...
The network is implemented based on the famous DL framework Tensorflow and trained by using a gtx 1080ti and gtx titan. The Adam optimizr is employed to train the network for up to 1000 epochs with 0.0002 learning rate, 0.7 decay rate, and 5 decay step. Besides, Dropout is introduced to prevent the models from overfitting. 
