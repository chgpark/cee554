...
class RONet:
    def __init__(self, args): # batch_size, input_size,sequence_length, hidden_size, output_size):
        ...
        self.keep_prob = args.dropout_prob
...
##################################################
    ...
    def set_multimodal_Preprocessing_bi_LSTM_for_8_uwb(self):
        with tf.variable_scope("preprocessing0"):
            cell_forward = tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_units = self.preprocessing_size)
            cell_forward = tf.nn.rnn_cell.DropoutWrapper(cell_forward, output_keep_prob= self.keep_prob)
            cell_backward = tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_units = self.preprocessing_size)
            cell_backward = tf.nn.rnn_cell.DropoutWrapper(cell_backward, output_keep_prob= self.keep_prob)
    ...
    def set_first_layer_bi_LSTM(self):
        with tf.variable_scope("Stacked_bi_lstm1"):
            ...
            cell_forward1 = tf.nn.rnn_cell.DropoutWrapper(cell_forward1, output_keep_prob= self.keep_prob)
            cell_backward1 = tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_units = self.first_layer_output_size)
            cell_backward1 = tf.nn.rnn_cell.DropoutWrapper(cell_backward1, output_keep_prob= self.keep_prob)
    ...
    def set_second_layer_bi_LSTM(self):
        with tf.variable_scope("Stacked_bi_lstm2"):
            ...
            cell_forward2 = tf.nn.rnn_cell.DropoutWrapper(cell_forward2, output_keep_prob= self.keep_prob)
            cell_backward2 = tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_units = self.second_layer_output_size)
            cell_backward2 = tf.nn.rnn_cell.DropoutWrapper(cell_backward2, output_keep_prob= self.keep_prob)
            # outputs : tuple
